...
public class Main {
    public static void main(String[] args) {
        ...
        // 여행
        bookList.stream()
                .filter(b -> b.getCategory().equals("여행"))
                .forEach(t -> System.out.println(t.getBookName() + "," + t.getCategory()));

        // 16200 이하
        bookList.stream()
                .filter(b -> b.getPrice() < 16200)
                .forEach(t -> System.out.println(t.getPrice()));

        // 경제
        bookList.stream()
                .filter(b -> b.getBookName().contains("경제"))
                .forEach(t -> System.out.println(t.getBookName()));
        // max
        ...
        double sum = bookList.stream()
                .filter(b -> b.getCategory().equals("IT"))
                .mapToDouble(Book::getPrice)
                .sum();
        System.out.println("sum = " + sum);

        // sale
        List<Book> discountList = bookList.stream()
                .filter(b -> b.getCategory().equals("IT"))
                .map(p -> {
                    p.setPrice(p.getPrice() * 0.6);
                    return p;
                })
                .toList();

        discountList.forEach(t -> System.out.println(t.getCategory() + ", " + t.getPrice()));
